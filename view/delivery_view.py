...
def format_package_line(package):
    if package is None:
        return "|" + " " * 54 + "|"
    return "| ID: {:>2} | STATUS: {:<9} | DELIVERY TIME: {:<8} |".format(
        *package.get("id", "status"), str(package.get("delivery_time"))
    )
...
def output_travel_data(trucks):
    ...
    for package_1, package_2, package_3 in zipped_logs:
        print(
            format_package_line(package_1)
            + format_package_line(package_2)
            + format_package_line(package_3)
        )
    ...
    for truck in trucks:
        for package in truck.get("log"):
            if package.get("id") == target_id:
                output = "| ID: {:>2} | Address: {:<} | Status: {:<9} | Delivery Time: {:<8} |".format(
                    *package.get("id", "address", "status"),
                    str(package.get("delivery_time")),
                )
