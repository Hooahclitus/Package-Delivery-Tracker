...
def create_package_data(file_path):
    ...
    keys = ['id', 'address', 'city', 'state', 'zip', 'deadline', 'weight', 'req_truck', 'delayed', 'truck_grp', 'wrong_addr', 'has_deadline', 'status', 'delivery_time']
    ...
    with open(file_path, mode="r", encoding="utf-8-sig") as file:
        ...
        for package in csv_reader:
            ...
            tbl = HashTable(13, *[item for key, val in zip(keys, [package[i] for i in range(12)] + ['Undelivered', 'N/A']) for item in (key, val)])
